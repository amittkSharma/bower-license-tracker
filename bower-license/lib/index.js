        ...
        bowerJson.find(path.resolve(options.directory, package), function(err, filename){
            ...
            bowerJson.read(filename, function(err, bowerData){
                ...
                if (bowerData.repository) {
                    moduleInfo.repository = bowerData.repository.url
                }
                moduleInfo.repository = moduleInfo.repository || bowerData.homepage || 'No Repo/Homepage Url found';
                moduleInfo.name = bowerData.name || 'No Info Found';
                moduleInfo.version = bowerData.version || 'No Info Found';
                moduleInfo.description = bowerData.description || 'No Description Found';

                var filesInFolder = fs.readdirSync(path.resolve(options.directory, package));
                var index = filesInFolder.indexOf('LICENSE')
                moduleInfo.licenseFile = 'No File Found';
                if(index >= 0) moduleInfo.licenseFile = path.resolve(options.directory, package) + '\\LICENSE';
                moduleInfo.licenseModified = 'No Info Found';
                ...
                npmLicense.init({start: path.resolve(options.directory, package)}, function(npmData){
                    ...
                    if (moduleInfo.licenses.length === 0){
                        ...
                        moduleInfo.licenses = _.uniq(moduleInfo.licenses).join(', ');
                    }
